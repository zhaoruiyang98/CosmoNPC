...
def run_task(statistic, correlation_mode, geometry, catalogs, **kwargs):
    ...
    if statistic == "pk":
        ...
        if k_max >= nyquist_freq:
            raise ValueError(
                f"k_max {k_max} is larger than the Nyquist frequency {nyquist_freq}. \
        ...
        if geometry == "survey-like":
            ...
            stat_attrs, rfield_a, rfield_b = get_mesh_pk_survey(
                catalogs,
                correlation_mode,
                nmesh=nmesh,
                geometry=geometry,
            ...
            if all(p % 2 == 1 for p in poles):
                raise ValueError(
                    f"All poles in {poles} are odd. Please choose at least one even pole, \
                                 since odd poles are defaultly set to 0 for periodic boxes."
                )

            stat_attrs, rfield_a, rfield_b = get_mesh_box(
                catalogs,
                correlation_mode,
                nmesh=kwargs["nmesh"],
                geometry=geometry,
        ...
        if rank == 0:
            logging.info(
                f"Time to create (FKP) overdensity field(s): {time_get_rfield - time_start:.2f} seconds"
            )
            logging.info(f"{'$' * 60} Start to compute power spectrum. {'$' * 60}")

        # Compute power spectrum
        if geometry == "survey-like":
            pk_res = calculate_power_spectrum_survey(
                stat_attrs, rfield_a, rfield_b, correlation_mode, comm=comm, **kwargs
            )
        else:
        ...
        if rank == 0:
            ...
            if correlation_mode == "auto":
                output_path = os.path.join(
                    output_dir, f"pk_res_{correlation_mode}_{catalog_name_a}.npy"
                )
            elif correlation_mode == "cross":
                output_path = os.path.join(
                    output_dir,
                    f"pk_res_{correlation_mode}_{catalog_name_a}_{catalog_name_b}.npy",
                )
        ...
        if rank == 0:
            logging.info(
                f"Time to compute power spectrum: {time_pk - time_get_rfield:.2f} seconds"
            )
    ...
    if statistic == "bk_sugi":
        ...
        if 2 * k_max >= nyquist_freq:
            raise ValueError(
                f"2 * k_max = {2 * k_max} is larger than the Nyquist frequency {nyquist_freq}. \
                             Please choose a smaller k_max."
            )
        ...
        if geometry == "box-like":
            ...
            stat_attrs, rfield_a, rfield_b, rfield_c = get_mesh_box(
                catalogs,
                correlation_mode,
                statistic=statistic,
                nmesh=kwargs["nmesh"],
        ...
        if rank == 0:
            logging.info(
                f"Time to create overdensity field(s): {time_get_rfield - time_start:.2f} seconds"
            )
            logging.info(
                f"{'$' * 60} Start to compute bispectrum using Sugiyama estimator. {'$' * 60}"
            )

        # Compute bispectrum using Sugiyama estimator

        if geometry == "box-like":
            bk_res = calculate_bk_sugi_box(
                rfield_a,
                rfield_b,
                rfield_c,
                correlation_mode,
            ...
            bk_res = calculate_bk_sugi_survey(
                rfield_a,
                rfield_b,
                rfield_c,
                correlation_mode,
        ...
        if rank == 0:
            logging.info(
                f"Time to compute bispectrum: {time_bk - time_get_rfield:.2f} seconds"
            )
            logging.info(
                f"Total time for bk_sugi task: {time_bk - time_start:.2f} seconds"
            )
